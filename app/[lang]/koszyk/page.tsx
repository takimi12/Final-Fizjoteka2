	...
	const validateDiscountCode = async () => {
		...
		try {
			...
			const foundCode = codes.find(
				(code: { code: string }) => code.code.toLowerCase() === discountCode.toLowerCase(),
			);
	...
	const onSubmit = async (data: FormData) => {
		...
		try {
			...
			const { data: paymentResponse } = await axios.post(
				`${process.env.NEXT_PUBLIC_APP_URL}/api/przelewy24`,
				paymentData,
			);
				...
				<div className={styles.cart}>
					<div className={styles.orderSummary}>
						...
						<div className={styles.discountCode}>
							...
							{appliedDiscount > 0 && (
								<p className={styles.discountApplied}>Rabat {appliedDiscount}% został naliczony</p>
							)}
						</div>
					</div>
				</div>
					...
					<div className={styles.paymentDetails}>
						...
						<label className={styles.label}>
							...
							{errors.nameAndSurname && (
								<span className={styles.errorText}>To pole jest wymagane</span>
							)}
						</label>

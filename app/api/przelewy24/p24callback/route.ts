...
export async function POST(request: NextRequest) {
	...
	if (result === false) {
		// Aktualizacja transakcji w przypadku błędu
		await Transaction.findOneAndUpdate(
			{ _id: body.sessionId },
			{ 
				status: false,
				errorDetails: "Transaction verification failed" // Tutaj można dostarczyć bardziej szczegółowy błąd
			}
		);
	...
	// Aktualizacja transakcji po poprawnej płatności
	await Transaction.findOneAndUpdate(
		{ _id: body.sessionId },
		{ 
			status: true,

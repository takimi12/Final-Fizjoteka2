...
async function getP24TransactionDetails(
    ...
    try {
        ...
        if (verifyResult === true) {
            ...
            p24Status.verificationStatus = "failed";
            p24Status.processingStatus = "error";
            p24Status.error = "Weryfikacja płatności nie powiodła się.";
        }
    } catch (error: any) {
        p24Status.error = "Błąd podczas przetwarzania płatności.";
        p24Status.processingStatus = "error";
        p24Status.isRejected = true;
    }
...
export async function GET(request: NextRequest) {
    try {
        ...
        const expectedAmount = transactionData.products.reduce(
            (acc: number, product: any) => acc + Number(product.price) * Number(product.quantity),
            0
        );

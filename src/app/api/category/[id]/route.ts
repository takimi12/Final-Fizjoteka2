...
export async function PUT(request: Request, { params }: { params: Params }) {
  const { id } = params;
  const { newTitle: title, newSubtitle1: subtitle1, newSubtitle2: subtitle2, newSubtitle3: subtitle3, newDescription: description, newPrice: price, newCategory: category, imageFileUrl } = await request.json() as PutRequestBody;
  await dbConnect();
